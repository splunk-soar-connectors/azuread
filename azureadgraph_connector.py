...
def _handle_login_response(request):
    ...
    try:
        ...
        return HttpResponse(f"{MS_AZURE_DECRYPTION_ERROR}: {e!s}", content_type="text/plain", status=400)
...
class AzureADGraphConnector(BaseConnector):
    ...
    def _process_empty_response(self, response, action_result):
        ...
        return RetVal(action_result.set_status(phantom.APP_ERROR, MS_AZURE_ERROR_EMPTY_RESPONSE.format(code=response.status_code)), None)
    ...
    def _handle_reset_password(self, param):
        ...
        data = {"passwordProfile": {"forceChangePasswordNextLogin": force_change}}
        if temp_password != "":
    ...
    def _handle_list_group_members(self, param):
        ...
        for item in response.get("value", []):
            url = item.get("url", "")
            match = re.match(AZUREADGRAPH_API_REGEX.format(AZUREADGRAPH_API_REGION[config.get(MS_AZURE_URL, "Global")], self._tenant), url)
        ...
        for item in user_object_ids:
            data = {"displayName": user_id_map[item], "objectId": item}
    ...
    def _get_token(self, action_result, from_action=False):
        ...
        if from_action or self._refresh_token is not None:
            ...
            try:
                ...
                self.debug_print(f"{MS_AZURE_DECRYPTION_ERROR}: {self._get_error_message_from_exception(e)}")
    ...
    def finalize(self):
        try:
            ...
            self.debug_print(f"{MS_AZURE_ENCRYPTION_ERROR}: {self._get_error_message_from_exception(e)}")
